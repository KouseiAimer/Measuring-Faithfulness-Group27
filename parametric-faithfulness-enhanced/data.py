...
def load_or_generate_dataset_cots(model_id, tokenizer, dataset_id, seed, temperature, force_generate=False, sentencize=True, atomic=False, max_instances=250, max_new_tokens=300):
    ...
    if not os.path.exists(floc) or force_generate:
        dataset_cots = generate_dataset_cots(model_id, tokenizer, dataset_id, temperature=temperature, sentencize=sentencize, max_instances=max_instances, max_new_tokens=max_new_tokens)
        cache_cots(dataset_cots, root,  short_model_id, dataset_id, seed, temp, sample_suffix=sample_suffix)
