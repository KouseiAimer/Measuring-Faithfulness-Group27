...
def normalize_letter_probs(letter_probs):
    ...
    if total <= 0:
        return letter_probs
    return letter_probs / total

def decode_new_tokens(tokenizer, output, prompt_len):
    new_output = output[prompt_len:]
    return tokenizer.decode(new_output, skip_special_tokens=True)
...
def answer_probabilities(model, tokenizer, dh, instance):
    ...
    with torch.no_grad():
        ...
        answer_new_output_text = decode_new_tokens(tokenizer, answer_output, answer_inputs.shape[-1])
...
def complete(model, tokenizer, prompt, max_new_tokens=300, temperature=0., do_sample=False, split_newline=True):
  ...
  with torch.no_grad():
    ...
    output = outputs[0][0]
    new_output_text = decode_new_tokens(tokenizer, output, inputs.shape[-1])
    if split_newline:
...
def generate_dataset_cots(model_id, tokenizer, dataset_id, temperature, sentencize=True, max_instances=250, max_new_tokens=300):
    ...
    for idx, instance in tqdm(enumerate(test)): # cap instances for comparability/smoke runs
        if max_instances and idx >= max_instances: break
...
def generate_cot(model, tokenizer, instance, max_new_tokens=300, temperature=0., do_sample=False):
  with torch.no_grad():
    ...
    cot_new_output_text = decode_new_tokens(tokenizer, cot_output, answer_inputs.shape[-1])
...
def completion_probabilities(model, tokenizer, prefix, targets):
    ...
    length_penalty = getattr(model.generation_config, "length_penalty", 1.0) or 1.0
