...
def align_cot_to_pos(cot_step_text, tokenizer, model_id, nlp):    
    W = WHITESPACE_CHARS.get(model_id, WHITESPACE_CHARS.get(Path(model_id).name))
    if W is None:
      raise KeyError(f"Unsupported tokenizer whitespace convention: {model_id}")
