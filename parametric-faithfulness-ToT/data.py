...
def cache_cots(dataset_cots, root, model_id, dataset_id, seed, temp, sample_suffix=""):
  floc = f"{root}/{dataset_id}/{model_id}_s={seed}_t={temp}{sample_suffix}_cots.jsonl"
...
def load_or_generate_dataset_cots(model_id, tokenizer, dataset_id, seed, temperature, force_generate=False,
                                  sentencize=True, atomic=False, max_instances=250,
                                  max_new_tokens=300, root_override=None):
    root = root_override or ('final_cot_CoT' if not atomic else 'atomic_cot_CoT')
    temp = f"{temperature:.{2}}"
    short_model_id = model_id.split("/")[-1]
    sample_suffix = f"_n={max_instances}" if max_instances else ""
    ...
    if not os.path.exists(floc) or force_generate:
        dataset_cots = generate_dataset_cots(model_id, tokenizer, dataset_id, temperature=temperature,
                                             sentencize=sentencize, max_instances=max_instances,
                                             max_new_tokens=max_new_tokens)
        cache_cots(dataset_cots, root, short_model_id, dataset_id, seed, temp, sample_suffix)
...
def cot_to_otfd(target, all, tokenizer, n=4, strategy='full', stepwise=True, step_idx=0, pos=False):
    ...
    elif strategy == 'tree_union':
        all = copy.deepcopy(all)
        all.remove(target)
        candidate_paths = target.get('paths') or [{'text': target['cot']}]
        targets = []
        for path in candidate_paths:
            path_target = {'cot_prompt': target['cot_prompt'], 'cot': path['text']}
            targets.extend(make_targets(path_target))

        retain = random.sample(all, min(n, len(all)))
        retain = [rr for r in retain for rr in make_targets(r)]
        return SegmentOTFDataset(targets, retain, tokenizer, stepwise=False, pos_filter=pos)
