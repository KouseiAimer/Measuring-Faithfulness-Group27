...
from data import FRCollator, cot_to_otfd, model_name_dict, load_or_generate_dataset_cots, load_jsonl
...
def unlearn_single(model_id, tokenizer, args, target, step_idx, cots_train, cots_verify, dh, instance_idx):
    ...
    if not trainable_params:
        raise ValueError("No trainable parameters selected for unlearning.")
...
def make_parser():
    ...
    parser.add_argument('--strategy', type=str, default='sentencize',
                        choices=['full', 'sentencize', 'tree_union'],
                        help="Which unlearning strategy to use.")
    ...
    parser.add_argument('--max_samples', type=int, default=70,
                        help="Number of reasoning instances to generate or read before splitting.")
    parser.add_argument('--n_unlearn', type=int, default=50,
                        help="Maximum number of target questions to unlearn.")
    parser.add_argument('--verify_samples', type=int, default=20,
                        help="Number of held-out same-task questions for specificity.")
    parser.add_argument('--cot_max_new_tokens', type=int, default=300,
                        help="Maximum number of generated tokens in initial reasoning.")
    parser.add_argument('--eval_max_new_tokens', type=int, default=300,
                        help="Maximum generated tokens for post-unlearning reasoning evaluation.")
    parser.add_argument('--max_steps_per_question', type=int, default=0,
                        help="Optional cap for smoke tests; 0 evaluates every reasoning step.")
    parser.add_argument('--reasoning_cache', type=str, default='',
                        help="Optional JSONL cache of CoT/ToT selected reasoning paths.")
    parser.add_argument('--reasoning_root', type=str, default='final_cot_CoT',
                        help="CoT generation cache root if --reasoning_cache is not supplied.")
    parser.add_argument('--result_root', type=str, default='final_result_CoT',
                        help="Directory root for JSONL unlearning results.")
    parser.add_argument('--split_manifest', type=str, default='splits/openbookqa_seed1001_n50_retain20.json',
                        help="Location to write the shared target/retain id partition.")
    parser.add_argument('--source_tag', type=str, default='cot',
                        help="Reasoning-source tag included in output metadata and filename.")
...
def main():
    ...
    if args.reasoning_cache:
      cot_data = load_jsonl(args.reasoning_cache)
    else:
      cot_data = load_or_generate_dataset_cots(model_id=model_id, tokenizer=tokenizer,
                                                ...
                                                atomic=args.atomic, max_instances=args.max_samples,
                                                max_new_tokens=args.cot_max_new_tokens,
                                                root_override=args.reasoning_root)

    # Shuffle data
    random.shuffle(cot_data)

    # "Specificity" split = same task, different instances
    N_verify = min(args.verify_samples, max(0, len(cot_data) - 1))
    ...
    if not split_path.exists():
      split_path.write_text(json.dumps({
          ...
          'target_ids': [row['id'] for row in cots_train[:args.n_unlearn]],
          'retain_ids': [row['id'] for row in cots_verify],
      }, indent=2), encoding='utf-8')
    else:
      existing_split = json.loads(split_path.read_text(encoding='utf-8'))
      expected_targets = [row['id'] for row in cots_train[:args.n_unlearn]]
      expected_retain = [row['id'] for row in cots_verify]
      if existing_split.get('target_ids') != expected_targets or existing_split.get('retain_ids') != expected_retain:
          raise ValueError("Reasoning cache does not match the shared CoT/ToT target-retain split.")
    ...
    # Logging
    if args.mmlu:
      root_name = f"{args.result_root}_mmlu"
      N_unlearn = args.mmlu
    elif args.gsm:
      ...
      root_name = args.result_root
      N_unlearn = args.n_unlearn
    
    resdir = f"{root_name}/{args.dataset}/{short_model}/"
    os.makedirs(resdir, exist_ok=True)
    # No POS, no ff2, unlearn full
    logfile_name = f"{args.source_tag}_{args.method}_{args.strategy}_s={args.stepwise}_lr={str(args.lr)}_rs={args.seed}_n={N_unlearn}_pos={args.pos}_ff2={args.ff2}.out"
    ...
    for idx, target in enumerate(cots_train[:N_unlearn]):
        if args.strategy == 'tree_union':
          target = copy.deepcopy(target)
          target['segmented_cot'] = [target['cot']]
        # Clunky for now
        n_steps = 1
        if args.stepwise:
          n_steps = len(target['segmented_cot'])
        if args.max_steps_per_question:
          n_steps = min(n_steps, args.max_steps_per_question)
        ...
        for step_idx in range(n_steps):
          ...
          instance_info['reasoning_source'] = args.source_tag
          if 'tot_mode' in target:
              instance_info['tot_mode'] = target['tot_mode']
              instance_info['paths'] = target.get('paths', [])
              instance_info['tree_metadata'] = target.get('tree_metadata', {})
