...
def normalize_letter_probs(letter_probs):
    total = letter_probs.sum()
    return letter_probs / total if total > 0 else letter_probs
...
def answer_probabilities(model, tokenizer, dh, instance):
    ...
    with torch.no_grad():
        ...
        answer_output = model.generate(input_ids=answer_inputs, attention_mask=torch.ones_like(answer_inputs), max_new_tokens=10,
                                        output_scores=True,
        ...
        letter_probs = normalize_letter_probs(first_token_probs[answer_indices])
...
def complete(model, tokenizer, prompt, max_new_tokens=300, temperature=0., do_sample=False,
             split_newline=True, top_p=1.0):
  ...
  with torch.no_grad():
    ...
    outputs = model.generate(input_ids=inputs, attention_mask=torch.ones_like(inputs), max_new_tokens=max_new_tokens,
                                    output_scores=True,
                                    do_sample=do_sample, return_dict_in_generate=True,
                                    pad_token_id=tokenizer.pad_token_id, **generation_kwargs)
...
def letter_completion(model, tokenizer, prompt, N):
  with torch.no_grad():
    ...
    answer_inputs = tokenizer.encode(prompt, padding=False, add_special_tokens=False, return_tensors='pt').to(device)
    answer_output = model.generate(input_ids=answer_inputs, attention_mask=torch.ones_like(answer_inputs), max_new_tokens=20,
...
def generate_dataset_cots(model_id, tokenizer, dataset_id, temperature, sentencize=True,
                          max_instances=250, max_new_tokens=300, split="test"):
    ...
    selected_split = valid if split == "validation" else test
    if dataset_id == 'sqa': selected_split = valid # SQA test doesn't have answers

    instance_info = []
    
    for idx, instance in tqdm(enumerate(selected_split)):
        ...
        segmented_cot = None
        if sentencize:
            segmented_cot = safe_sent_tokenize(cot)
        inst_details = {
...
def generate_cot(model, tokenizer, instance, max_new_tokens=300, temperature=0., do_sample=False):
  with torch.no_grad():
    ...
    cot_output = model.generate(input_ids=answer_inputs, attention_mask=torch.ones_like(answer_inputs), max_new_tokens=max_new_tokens,
                                    output_scores=True,
...
def cot_generate(model, tokenizer, instance, max_new_tokens=300, temperature=0., do_sample=False):
  with torch.no_grad():
    ...
    answer_inputs = tokenizer.encode(answer_prompt, padding=False, add_special_tokens=False, return_tensors='pt').to(device)
    # 2. Generate answer
    answer_output = model.generate(input_ids=answer_inputs, attention_mask=torch.ones_like(answer_inputs),
                                    max_new_tokens=10,
                                    output_scores=True,
                                    temperature=temperature, do_sample=do_sample,return_dict_in_generate=True,
                                    pad_token_id=tokenizer.pad_token_id) # , num_return_sequences=10
